...
enum EnumTxEtapas txEtapa = FAZ_NADA;
	...
	case TA1IV_TACCR2:  //canal 2, //Vem pra cá a cada intervalo de tempo definido pelo ta1ccr2
		if (e == 0){//estado de decisao, se envia string ou sequencia de dados RAWs
				...
				if (pc != 0) {//tenho string vindo da fila?//
					IE2 |= UCA0TXIE; e = 1; txEtapa = BREAK;//tendo, imprima-a
...
__interrupt void USCI0TX_ISR(void){	//1) versão com break;
	...
	case ID:
		...
		break;
	case BREAK:// É necessário enviar um "dummy byte" para disparar a sequência// UCA0TXBUF = 0x00;
		/*UCA0CTL1 |= UCTXBRK;*/ UCA0TXBUF = 0x55; txEtapa = ID;
		break;
	case FAZ_NADA:
		break;
	}//switch
